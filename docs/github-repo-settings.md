...
Only squash merging is allowed onto `main`, keeping one commit per PR — consistent with the conventional-commit style commitlint enforces. There is no release-automation workflow, so every squashed commit on `main` is its own unit of deployment (see `deployment-environments.md`).
...
This repo runs a single GitHub Environment.
...
The environment doesn't restrict which branches/tags can deploy to it at the GitHub level — that gating is done in workflow `if:` conditions instead (see `deployment-environments.md`). The approval flow prod's reviewer triggers is covered there too; this table is just the current GitHub-side config.
