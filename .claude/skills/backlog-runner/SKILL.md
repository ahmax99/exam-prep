...
- **Copy local env files in (repo-specific, after the skill returns).**
  ...
  `/implement` step runs in it, copy this repo's gitignored `.env` from the
  main worktree into the same relative path in the new worktree:
  `apps/nextjs/.env`. A worktree only contains tracked files, so
  this gitignored file is otherwise absent — and `/implement`'s gates
  (`bun run check-types`, the dev server) need it. This is a filesystem copy
  into an already-gitignored destination; never `git add` it.
