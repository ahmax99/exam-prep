...
- `security-reviewer` — Zod-at-the-boundary, server-only data access (no DB/S3 credentials reaching the browser), injection, secrets, S3 scoping. Emits **Security**. (This app has no authentication/authorization layer — if a diff adds one, this is the reviewer to sharpen first.)
...
- Plans and specs use acceptance criteria — "works correctly" is not testable; "returns 400 when the upload request omits a content type" is.
...
The `.claude/commands/` files are **project-specific orchestrators** — they encode this codebase's conventions (neverthrow `ResultAsync`, Zod at the boundary, feature-folder placement, server-only data access) and delegate to subagents in `.claude/agents/`. Use them when working on this repo.
...
- **`next-devtools`** (MCP, `.mcp.json`) — Next.js-specific introspection for `apps/nextjs`: `nextjs_docs` for version-current App Router / Cache Components docs, `nextjs_index` to map the route tree, `browser_eval` for a quick in-page check. Prefer it over `context7` for questions about Next.js itself (narrower and version-aware); `context7` stays the tool for every other library.
...
- **`context7`** (MCP) — fetch _current_ docs for any library (Next.js, Prisma, Zod, Tailwind) instead of relying on the training cutoff. Use anytime; especially before applying an unfamiliar API.
...
- **`neon`** (MCP, `.mcp.json`) — Neon's hosted remote MCP server (`mcp-remote` proxying `https://mcp.neon.tech/mcp`, OAuth-authenticated on first connect — no API key committed, since this is a public repo). Use it to inspect/manage the actual Neon project backing `DATABASE_URL` (branches, connection strings, query execution, migrations) directly from a session, instead of shelling out to `psql`/the Neon console. It talks to the same Postgres project `src/lib/prisma.ts` connects to at runtime — prefer `prisma migrate dev` (`.claude/rules/architecture.md`) for schema changes; reach for this MCP server for inspection, branch management, and ad-hoc queries.
