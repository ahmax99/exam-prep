...
- `.claude/agents/security-reviewer.md` — Zod-at-the-boundary, server-only data access (no DB/S3 credentials reaching the browser), injection, secrets, S3 scoping. (No auth/authorization layer exists in this app; flag one being added without a deliberate design.)
- `.claude/agents/correctness-reviewer.md` — the neverthrow `Result` flow, layer boundaries (logic in `server/api`, thin route handlers), Prisma queries/transactions/pagination, type safety (`any` is an oxlint error), atomic-design and feature-folder placement, code style rules.
