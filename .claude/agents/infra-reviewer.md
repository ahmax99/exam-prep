...
- **Destructive changes:** renaming a resource address, changing an immutable argument (e.g. S3 bucket name), or removing a module instantiation forces replacement or destroy of stateful resources. Call these out explicitly — the PR plan comment is the proof, but the reviewer flags the risk first. Suggest `moved {}` blocks for renames.
...
- Providers pinned in `versions.tf`; variables have `type` + `description`; no `default` on a `variable` block (see `.claude/rules/infra.md`) — this repo manages a single environment, so a new externally-supplied variable gets an entry in `vars/tfvars.example`, not a default.
